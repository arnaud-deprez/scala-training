...
object Chapter9 extends App {
	...
	// 2
	{
		...
		val out = new java.io.PrintWriter("target/data2.tab")
	...
	{
		import io.Source
		val file = Source.fromFile("src/main/scala/be/arndep/scala/fortheimpatient/Chapter5_Java.java").mkString
	...
	}
	{
		...
		hal.addFriend(meth).addFriend(bob)

		val trex = new Person("T-Rex Bones", 2580450)
		val people = List(hal, meth, bob, trex)
		println(people)
		import java.io._
		val out = new ObjectOutputStream(new FileOutputStream("target/ObjectStore"))
		out.writeObject(people)
		out.close()
	}
	{
		import java.io._
		val in = new ObjectInputStream(new FileInputStream("target/ObjectStore"))
